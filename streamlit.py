...
for model, model_str in zip(models, models_str):
    model.load_state_dict(torch.load("./results/{}".format(model_str)+("A" if model_str.startswith("ResNet") else "")))
    append = pd.DataFrame([[model_str, sum(p.numel() for p in model.parameters())]], columns=['Models', '# params'])
    df = pd.concat([df, append], ignore_index=True)
st.write(df)

models_option = df['Models'].unique().tolist()
models_selectbox = st.multiselect("Which model do you want to see ?", models_option, models_option)
print(df)
df_temp = df[df["Models"]==models_selectbox].copy()
print(df_temp)

fig = px.scatter(df_temp, x="Models", y=np.zeros(len(df_temp['# params'])), size=df_temp['# params'].tolist(), color='# params')
st.write(fig)