...
for epoch in range(EPOCHS):
    ...
    st.write(
        "{} --- Epoch {}/{} --- train loss : {} | train acc : {} | test loss : {} | test acc {} | time spent {}s".format(
            name,
            epoch + 1,
            EPOCHS,
            np.round(training_loss, 2),
            np.round(training_acc, 2),
            np.round(test_loss, 2),
            np.round(test_acc, 2),
            np.round(diff_time, 2),
        )
    )
